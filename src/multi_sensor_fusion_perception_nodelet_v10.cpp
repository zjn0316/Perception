// v2,2025/12/13
#include "multi_sensor_fusion_perception_nodelet_v10.h"
...
void MultiSensorFusionPerceptionNodelet::onInit() {
    ...
    static DeepsortLogger logger;  // 使用静态确保生命周期
    ...
    const people_tracker::PersonArray::ConstPtr& leg_msg)
{
    ...
    uwb_conf_ = CalcUwbConfidence(fp_rssi, rx_rssi, dis);
    ...
    uwb_pt_nlink.point.z = 0.54;// nlink坐标系下uwb标签大约高度 0.54 m
    ...
    std::vector<DetectBox> dets_deepsort = ToDetectBox(res_fd); // 转换为 DeepSort 格式
    ...
    for (size_t i = 0; i < dets_deepsort.size(); ++i) {
        ...
        MaskDepthStats stats = CalcMaskDepthStats(yoloDet.maskMatrix, depth_zed,cv::Rect(d.x1, d.y1, d.x2-d.x1, d.y2-d.y1));
    ...
    PrintSituations(uwb_conf_,leg_conf_,zed_conf_,target_uwb_,target_leg_,target_zed_);

    // fusion
    PreparTensor(target_uwb_, target_leg_, target_zed_, uwb_conf_,   leg_conf_,   zed_conf_);
    FusionInfer();
    ROS_INFO("\033[1;34m------------------Spin cost %d ms-----------------.\033[0m", static_cast<int>((t8-t0).toSec() * 1000));
}



// =========================================== 话题 ================================================
void MultiSensorFusionPerceptionNodelet::LoadTopic() {
...
// 打印参数配置的函数
void MultiSensorFusionPerceptionNodelet::PrintTopic() {
    ROS_INFO("============== 订阅 =============");
...
std::vector<DetectBox> MultiSensorFusionPerceptionNodelet::ToDetectBox(const std::vector<Detection>& src)
{
...
MaskDepthStats MultiSensorFusionPerceptionNodelet::CalcMaskDepthStats(const std::vector<float>& maskMatrix,
...
// =========================================== fusion ================================================
void MultiSensorFusionPerceptionNodelet::CollectDataset()
{
    // 每 L 帧换一个新 seq 目录
    constexpr size_t L = 256;
    if (g_frames_in_seq % L == 0)  g_seq_id++;

    // 两级目录：~/data/raw_npz/seqXXX/
    const char* parent = "/home/zjn/temp/py_ws/sensor_fusion_nn/data/raw_npz";
    mkdir(parent, 0755);
    char seq_dir[512];
    std::snprintf(seq_dir, sizeof(seq_dir), "%s/seq%03llu", parent, g_seq_id);
    mkdir(seq_dir, 0755);

    // 写盘
    char fname[512];
    std::snprintf(fname, sizeof(fname), "%s/%06lu.npz",
                  seq_dir, g_frames_in_seq % L);
    cnpy::npz_save(fname, "x", seq_buf_.data(), {1, 12}, "w");
    ROS_WARN("Dataset >>> %s", fname);

    g_frames_in_seq++;
}

void MultiSensorFusionPerceptionNodelet::PreparTensor(
    ...
    float uwb_conf, float leg_conf, float zed_conf)
{
    ...
    // 想采集数据时，把 CollectDataset() 的注释去掉即可
    CollectDataset();
}

void MultiSensorFusionPerceptionNodelet::FusionInfer()
{
    ...
    if (fusion_->infer(seq_buf_, out))           // ← 这里失败就静默返回
    {
        ...
        ROS_INFO("fusion:\t x=%.2f, y=%.2f, z=%.2f, conf=%.2f",fused.point.x,fused.point.y,fused.point.z,fused_conf);
...
// =========================================== 工具函数 ================================================
void MultiSensorFusionPerceptionNodelet::PrintSituations(float uwb_conf, float leg_conf, float zed_conf ,
...
{
    ROS_INFO("UWB:\t x=%.2f, y=%.2f, z=%.2f, conf=%.2f",
              uwb_pt_base.point.x, uwb_pt_base.point.y, uwb_pt_base.point.z, uwb_conf);
    ROS_INFO("Leg:\t x=%.2f, y=%.2f, z=%.2f, conf=%.2f",
              leg_pt_base.point.x, leg_pt_base.point.y, leg_pt_base.point.z, leg_conf);
    ROS_INFO("ZED:\t x=%.2f, y=%.2f, z=%.2f, conf=%.2f",
              zed_pt_base.point.x, zed_pt_base.point.y, zed_pt_base.point.z, zed_conf);
}




void MultiSensorFusionPerceptionNodelet::PrintTransformMatrix(
