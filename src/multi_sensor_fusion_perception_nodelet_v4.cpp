// v2,2025/12/4
#include "multi_sensor_fusion_perception_nodelet_v4.h"
    ...
    const people_tracker::PersonArray::ConstPtr& leg_msg)
{
    ...
    // print_transform_matrix("nlink", "zed2_left_camera_frame");
    // print_transform_matrix("zed2_left_camera_frame", "zed2_left_camera_optical_frame");

    
    // 处理各个传感器回调
    /* ---------- 1. UWB 处理（原 uwbCallback）---------- */
    {
        /* -------------- 1. 有效性检查 -------------- */
...
uwb_done:;
    ...
    /* ---------- 2. ZED 处理（原 zedCallback）---------- */
    {
        ...
        /* -------------- 2. YOLO 检测 + 过滤 10ms -------------- */
        std::vector<Detection> res    = detector_->inference(img_zed);
