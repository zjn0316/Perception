...
#include "frid_tracker.h"
#include "frid_fast-reid.h"
...
private:
    ...
    std::vector<DetectRes> toDetectRes(const std::vector<Detection>& yolo_dets);
    void drawID(cv::Mat& img,const std::vector<DetectRes>& dets,const std::vector<int>& ids);
