// v2,2025/12/11
#include "multi_sensor_fusion_perception_nodelet_v8.h"
    ...
    const people_tracker::PersonArray::ConstPtr& leg_msg)
{
    ...
    for (size_t i = 0; i < leg_msg->people.size(); ++i)
    {
        ...
        leg_uv_ = camera2pixel(leg_pt_cam, K);// leg 在 pixel 下的坐标
        printLegPositions(leg_pt_odom, leg_pt_base, leg_pt_cam, leg_uv_);
    ...
    // 画 Leg 绿点
    cv::circle(img_seg,cv::Point(static_cast<int>(leg_uv_.x),static_cast<int>(leg_uv_.y)),4, cv::Scalar(0, 255, 0), -1);
...
void MultiSensorFusionPerceptionNodelet::print_confidences(float uwb_conf, float vis_conf ,float leg_conf)
{
   ROS_DEBUG("UWB Confidence = %.2f", uwb_conf);
   ROS_DEBUG("ZED Confidence = %.2f", vis_conf);
